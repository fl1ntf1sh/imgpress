...
impl PdfRenderer for MuPdfRenderer {
    ...
    fn render_bytes(&self, bytes: &[u8]) -> Result<Vec<DynamicImage>> {
        let doc = mupdf::Document::from_bytes(bytes, "application/pdf")
            .map_err(|e| Error::Pdf(format!("failed to open: {}", e)))?;
        render_document(doc)
    }
}

fn render_document(doc: mupdf::Document) -> Result<Vec<DynamicImage>> {
    let page_count = doc
        .page_count()
    ...
    for i in 0..page_count {
        ...
        let pixmap = match page.to_pixmap(&matrix, &mupdf::Colorspace::device_rgb(), false, false) {
            Ok(p) => p,
            Err(e) => {
