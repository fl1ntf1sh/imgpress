...
--organize-after-success    全部成功后按姓名整理输出根目录文件
...
- 勾选“成功后整理文件”时，压缩全部成功后会按姓名归类输出根目录文件
...
勾选“成功后整理文件”或使用 `--organize-after-success` 时，仅在全部文件处理成功后整理输出根目录中的文件：

- 处理结果默认先直接写入输出根目录，不保留源文件夹中的目录结构
- 文件夹中的文件会带上文件夹名前缀，例如 `张三/照片.jpg` 输出为 `张三_照片.jpg`
- 多页 PDF 会带上源格式和页码，例如 `张三/材料.pdf` 输出为 `张三_材料_pdf_page1.jpg`
- 只处理输出根目录下的文件，不递归处理已有文件夹中的文件
- 根据文件名中第一个分隔符前的内容提取姓名；如果没有分隔符，则使用整个文件名 stem
- 如果输出根目录已经有对应姓名文件夹，则将文件移入该文件夹
- 如果没有对应姓名文件夹，则创建该文件夹后移入
